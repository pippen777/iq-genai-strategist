...
def apply_iq_branding():
    ...
    div.stButton > button:hover {
        ...
        box-shadow: 0 10px 20px rgba(142, 45, 226, 0.3) !important;
    }
...
if check_password():
    ...
    # 5. STEP 2: INDUSTRY (NEW BUTTONS)
    if "maturity" in st.session_state:
        st.markdown(f"**Maturity:** `{st.session_state.maturity}`")
        ...
        with i_col3:
            if st.button("📡 Telco"): st.session_state.ind = "Telecommunications"
        with i_col4:
            if st.button("🏛️ Public"): st.session_state.ind = "Public Sector"
        with i_col5:
            if st.button("⛏️ Mining"): st.session_state.ind = "Manufacturing & Mining"
