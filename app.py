...
if "ind" in st.session_state:
    ...
    if st.button("ORCHESTRATE STRATEGY", type="primary"):
        ...
        if "ERROR" in res: st.error(res)
            ...
            st.markdown(f'<div class="vision-container"><h1 style="font-weight:300; font-size: 2.5rem;">{extract_section(res, "[VISION]", "[KPI]")}</h1></div>', unsafe_allow_html=True)
            st.markdown(f'<div style="text-align:center; margin-bottom:60px;"><p class="label-accent">STRATEGIC IMPACT PROJECTION</p><h2 style="color:#00ADEF; font-size:3rem; font-weight:800;">{extract_section(res, "[KPI]", "[M1]")}</h2></div>', unsafe_allow_html=True)
            
            c1, c2, c3 = st.columns(3)
            with c1: st.markdown(f'<div class="phase-card"><p class="label-accent">PHASE 01 / FOUNDATION</p>{extract_section(res, "[M1]", "[M2]")}</div>', unsafe_allow_html=True)
            with c2: st.markdown(f'<div class="phase-card"><p class="label-accent">PHASE 02 / VELOCITY</p>{extract_section(res, "[M2]", "[M3]")}</div>', unsafe_allow_html=True)
            with c3: st.markdown(f'<div class="phase-card"><p class="label-accent">PHASE 03 / AGENTIC</p>{extract_section(res, "[M3]")}</div>', unsafe_allow_html=True)
